...
class StyleGANGeneratorNet(nn.Module):
  ...
  def __init__(self,
    ...
    self.mid_resolution = mid_resolution
...
class SynthesisModule(nn.Module):
  ...
  def __init__(self,
    ...
    self.resolution = resolution
    self.final_res_log2 = int(np.log2(self.resolution)) #9
    self.mid_resolution = mid_resolution
    self.mid_res_log2 = int(np.log2(self.mid_resolution))
    self.w_space_dim = w_space_dim
    self.fused_scale = fused_scale
    self.fmaps_base = fmaps_base
    self.fmaps_max = fmaps_max
  ...
  def forward(self, w, x=None, synthesisType=None):
    ...
    if synthesisType == None:
      ...
      return image
    elif synthesisType == 1:
      for res_log2 in range(self.init_res_log2, self.mid_res_log2):
        block_idx = res_log2 - self.init_res_log2
        if block_idx == 0:
          x = self.__getattr__(f'layer{2 * block_idx}')(w[:, 2 * block_idx])
        else:
        ...
        x = self.__getattr__(f'layer{2 * block_idx + 1}')(
            x, w[:, 2 * block_idx + 1])
      return x
    elif synthesisType == 2:
      for res_log2 in range(self.mid_res_log2, self.final_res_log2 + 1):
        if res_log2 + lod <= self.final_res_log2:
          block_idx = res_log2 - self.init_res_log2
          if block_idx == 0:
            x = self.__getattr__(f'layer{2 * block_idx}')(w[:, 2 * block_idx])
...
class SynthesisPostModule(SynthesisModule):
  """docstring for SynthesisPostModule"""
  def __init__(self,                
               init_resolution=4,
               resolution=512,
               w_space_dim=512,
    ...
    super().__init__(
               ...
               fmaps_max)
    self.mid_resolution = mid_resolution
    self.mid_res_log2 = int(np.log2(mid_resolution))

  def forward(self, x, w):
    print("here i am in the synthesis post network")
    if not (len(w.shape) == 3 and
            w.shape[1] == self.num_layers and
            w.shape[2] == self.w_space_dim):
      raise ValueError(f'The input tensor should be with shape [batch_size, '
...
class SynthesisPreModule(SynthesisModule):
  """docstring for SynthesisPostModule"""
  def __init__(self,                
               init_resolution=4,
               resolution=512,
  ...
  def forward(self, w):
    print("here i am in the synthesis pre network")
    if not (len(w.shape) == 3 and
            w.shape[1] == self.num_layers and
            w.shape[2] == self.w_space_dim):
      raise ValueError(f'The input tensor should be with shape [batch_size, '
    ...
    print(x.shape)
    lod = self.lod.cpu().tolist()
    for res_log2 in range(self.init_res_log2, self.mid_res_log2 + 1):
        block_idx = res_log2 - self.init_res_log2
        if block_idx == 0:
          x = self.__getattr__(f'layer{2 * block_idx}')(w[:, 2 * block_idx])
        else:
